...
/**
 * REST controller for managing {@link fr.uga.ecom.bookstore.domain.Order}.
 */
@RestController
@RequestMapping("/api")
public class OrderResource {
    ...
    /**
     * @author : Emez
     * {@code GET  /cart/:userId} : get user's Order if the status is IN CART".
     * or create a new Order with status IN_CART if none exists
     *
     * @param userId the id of the user whose panier (orderId) we want to retrieve.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the orderId, or with status {@code 404 (Not Found)}.
     */
    @GetMapping("/cart/{userId}")
    public ResponseEntity<OrderDTO> getCart(@PathVariable String userId) {
        log.debug("REST request to get the Order (if any) still in cart ");
        Optional<OrderDTO> orderDTO = orderService.getUserCart(Long.valueOf(userId));
        if (orderDTO.isPresent())
            return ResponseUtil.wrapOrNotFound(orderDTO);

        // otherwise create new Order for the user
        OrderDTO newOrder = new OrderDTO();
        newOrder.setCustomerId(Long.valueOf(userId));
        newOrder.setOrderStatus(OrderStatus.IN_CART);
        newOrder.setTotalPrice((float) 0);

        OrderDTO savedOrder = orderService.save(newOrder);
        return this.getOrder(savedOrder.getId());
    }


    /**
     * {@code GET  /order-history} : get orders history of a user
     *
     * @param pageable the pagination information.
     * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of orders in body.
     */
    @GetMapping("/order-history/{userId}")
    public ResponseEntity<List<OrderDTO>> get(@PathVariable Long userId, Pageable pageable) {
        log.debug("REST request to get a page of History Order {} ", userId);
        Page<OrderDTO> page = orderService.findOrderHistory(userId, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
